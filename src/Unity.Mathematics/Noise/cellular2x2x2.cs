...
namespace Unity.Mathematics
{
    public static partial class noise
    {
        ...
        public static float2 cellular2x2x2(float3 P)
        {
            ...
            float4 Pfx = Pf.x + math.float4(0.0f, -1.0f, 0.0f, -1.0f);
            float4 Pfy = Pf.y + math.float4(0.0f, 0.0f, -1.0f, -1.0f);
            float4 p = permute(Pi.x + math.float4(0.0f, 1.0f, 0.0f, 1.0f));
            p = permute(p + Pi.y + math.float4(0.0f, 0.0f, 1.0f, 1.0f));
            float4 p1 = permute(p + Pi.z); // z+0
            float4 p2 = permute(p + Pi.z + math.float4(1.0f,1.0f,1.0f,1.0f)); // z+1
